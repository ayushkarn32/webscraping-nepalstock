...
foreach($html->find('tr') as $data) {
     if($data->find('td', 0)->plaintext!=null){
         ...
         $item['minprice'] = $data->find('td[4]', 0)->plaintext;
         $item['closingprice'] = $data->find('td[5]', 0)->plaintext;
         $item['tradedshares'] = $data->find('td[6]', 0)->plaintext;
         $item['amount'] = $data->find('td[7]', 0)->plaintext;
         $item['previousclosing'] = $data->find('td[8]', 0)->plaintext;
         $item['difference'] = $data->find('td[9]', 0)->plaintext;
         $alldata[] = $item;
     }
}
